...
def are_identical_symbols_same_objects?
  ...
  hash = { symbol: "value" }
  hash[:symbol] = "new value"
  hash[:symbol].equal? hash[:symbol]
end

def are_identical_strings_same_objects?
  # TODO: Answer the question by making this method to return true or false
  hash = { symbol: "value" }
  hash[:new_symbol] = "value"
  hash.select { |_key, value| value == "value" }
  hash[:new_symbol].equal? hash[:symbol]
end
